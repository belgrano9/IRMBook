...
def test_vasicek_model_creation():
    ...
    assert isinstance(
        ...
        ModelFactory.create_model(
            "Vasicek", r0=0.05, k=0.3, theta=0.05, sigma=0.02, T=1, N=252, n_paths=1000
        ).__class__,
    )
...
""" def test_vasicek_zero_coupon_bond_pricing():
    vasicek = ModelFactory.create_model(
        "Vasicek", r0=0.05, k=0.3, theta=0.05, sigma=0.02, T=2, n_path=1, N=252
    )
    bond_price = vasicek.zero_coupon_bond_price(r=0.05)
...
def test_vasicek_parameter_variations(r0, k, theta, sigma):
    vasicek = ModelFactory.create_model(
        "Vasicek", r0=r0, k=k, theta=theta, sigma=sigma, T=1, N=252, n_paths=100
    )
    simulated_rates = vasicek.simulate()
